    ...
    def get_event(self, ev_id):
        ...
        if self.show_light:
            ...
            if len(light) > 0:
                ...
                light_wvfms_nat = self.light_wvfms[light_wvfm_ref]["samples"] - light_wvfm_peds
                last_layer_1 = light_wvfms_nat.astype(np.float32)
                last_layer_1[:,0,:,:] *= 10
                last_layer_1[:,2::2,:,:] *= 1.666
                not_three = [0,1,2,3,4,5]
                not_two = [0,1,2,3,6,7]
                not_one = [0,1,4,5,6,7]
                not_zero = [2,3,4,5,6,7]
                last_layer_1[:,not_two,:,:] *= 0.0
                light_wvfms = last_layer_1.astype(np.int64)
        ...
        for i,j in itertools.product(range(light_wvfms[0].shape[0]),range(light_wvfms[0].shape[1])):
            ...
            for k in range(len(self.geometry.attrs['module_RO_bounds'])):
                ...
                if found_x ==1:
                    ...
                    self.ax_bdv.plot_surface(light_z,light_x,light_y,color=cmap_value, alpha=0.4, shade=False)
                    if self.show_event_mx2:
