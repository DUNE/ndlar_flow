...
class Module0FlowEventDisplay:

    '''
        Class to set up interactive single module displays for files run through module0_flow.
        ...
        evd = Module0FlowEventDisplay(filedir=d, filename=f, geometry_file=g,nhits=1)
        test_evd.run()
    '''

    def __init__(self, filedir, filename, geometry_file=None, nhits=1):
    ...
    def run(self):
        ...
        while True:
            ...
            if not user_input:
                ...
                plt.savefig(self.filename+'_Event_'+str(ev_id)+'_using_module0_flow_hits.pdf')
            else:
                try:
                    clear_output(wait=True)
    ...
    def get_event_start_time(self, event):
        ...
        # charge multiplied by 0.25 everywhere to match proto_nd_flow calib hits dataset units
        t0_charge_threshold = 200.0 *0.250# Rough qsum threshold
        ev_id = event['id']
        ...
        if (max_ts - min_ts) > ticks_per_qsum:
            ...
            q_vs_t = np.histogram(hits['ts'],
                                  bins=time_bins,
                                  weights=hits['q']*0.250)[0] # charge multiplied by 0.25 everywhere to match proto_nd_flow calib hits dataset units
    ...
    def display_event(self, ev_id):
        ...
        norm = matplotlib.colors.Normalize(
            vmin=min(self.hits[hit_ref]['q'] * 0.250), vmax=max(self.hits[hit_ref]['q'] * 0.250))
        ...
        if 'ntracks' in event.dtype.name and event['ntracks']:
            ...
            for i, track in enumerate(tracks):
                ...
                self.ax_xy.scatter(hits_trk['px'], hits_trk['py'], lw=0.2, ec='C{}'.format(
                    i+1), c=cmap(norm(hits_trk['q'] *0.250)), s=5, alpha=0.75)

                hit_zvals = [self._get_z_coordinate(io_group, io_channel, time) for io_group, io_channel, time in zip(
                    hits_trk['iogroup'], hits_trk['iochannel'], hits_trk['ts']-track['t0'])]

                self.ax_zy.scatter(hit_zvals, hits_trk['py'], lw=0.2, ec='C{}'.format(
                    i+1), c=cmap(norm(hits_trk['q']* 0.250)), s=5, alpha=0.75)
                self.ax_xyz.scatter(hits_trk['px'], hit_zvals, hits_trk['py'], lw=0.2, ec='C{}'.format(
                    i+1), c=cmap(norm(hits_trk['q'] * 0.250)), s=5, alpha=0.75)
        ...
        for itrk, (ts, te) in enumerate(zip(track_start, track_end)):
            ...
            self.ax_xyz.scatter(hits_trk['px'], hits_drift_trk['z'], hits_trk['py'], lw=0.2, ec='C{}'.format(
                itrk+1), c=cmap(norm(hits_trk['q'] * 0.250)), s=5, alpha=0.75)
        ...
        if np.any(unassoc_hit_mask):
            ...
            self.ax_xyz.scatter(unassoc_hits['px'], hit_zvals, unassoc_hits['py'], lw=0.2, ec='C0', c=my_cmap(
                norm(unassoc_hits['q']  * 0.250)), s=5, alpha=0.75)
            self.ax_xy.scatter(unassoc_hits['px'], unassoc_hits['py'], lw=0.2, ec='C0', c=my_cmap(
                norm(unassoc_hits['q']  * 0.250)), s=5, alpha=0.75)
            self.ax_zy.scatter(hit_zvals, unassoc_hits['py'], lw=0.2, ec='C0', c=my_cmap(
                norm(unassoc_hits['q']  * 0.250)), s=5, alpha=0.75)
