...
class T0Reconstruction(H5FlowStage):
    '''
        ...
        Parameters:
         ...
         - ``light_hits_dset_name``: ``str``, path to input light hits dataset
         - ``ext_trigs_dset_name``: ``str``, path to input external trigger dataset

         Both ``light_hits_dset_name`` and ``ext_trigs_dset_name`` are required in the cache.
    ...
    default_light_hits_dset_name = 'light/hits'
    default_ext_trigs_dset_name = 'charge/ext_trigs'
    ...
    def init(self, source_name):
        ...
        self.data_manager.set_attrs(self.t0_dset_name,
            ...
            type_lookup=np.array([tuple([t for t in self.t0_type.values()])], dtype=np.dtype([(t,'u1') for t in self.t0_type.keys()]))
            )
    ...
    def run(self, source_name, source_slice, cache):
        ...
        if len(events):
            ...
            # if external trigger, use min(trigger_ts) - 2 as t0, type == 1
            ...
            # if light hits present, use rising edge of largest signal, type == 2
            # light/t_ns + busy_ns + rising_spline
            ts = light_hits['ns'] + light_hits['busy_ns'] + light_hits['rising_spline']
            ts_err = light_hits['rising_err_spline']

            ts = ts * (resources['Units'].ns / resources['RunData'].crs_ticks) # ns -> crs ticks
            ts_err = ts_err * (resources['Units'].ns / resources['RunData'].crs_ticks) # ns -> crs ticks
            ...
            # if no external trigger, use start timestamp, type == 0
