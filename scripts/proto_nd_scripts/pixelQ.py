...
ymax, zmax = np.amax(positions, 0)
ymin, zmin = np.amin(positions, 0)

ymax, zmax = ceil(ymax), ceil(zmax)
ymin, zmin = floor(ymin), floor(zmin)

print(ymax, zmax)
print(ymin, zmin)

outfilename = "/pscratch/sd/l/lzazueta/pixelQ_iogroup"+str(io_group)+"_data_290files_angle.root" 
...
for n in range(1,145):
    ...
    for index, track in enumerate(hits):
        ...
        hits_group = track[track['io_group']==io_group]
