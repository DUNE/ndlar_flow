    ...
    for file in glob.glob(file_dir+'/*.h5'): # Loop over files files
        ...
        for pdg in sel_event_pdgs:
            ...
            for event_id in sel_event_ids:
                ...
                track_charge_data = tracks[track_ref]['q'][0]
                track_length_data = tracks[track_ref]['length'][0]
                track_num_hits_data = tracks[track_ref]['nhit'][0]
                track_theta_data = tracks[track_ref]['theta'][0]
                track_phi_data = tracks[track_ref]['phi'][0]
                track_ts_start_data = tracks[track_ref]['ts_start'][0]
                track_ts_end_data = tracks[track_ref]['ts_end'][0]
                track_dx_data = tracks[track_ref]['dx'][0]
                track_dq_data = tracks[track_ref]['dq'][0]
                track_start_pt_data = tracks[track_ref]['start'][0]
                track_end_pt_data = tracks[track_ref]['end'][0]
