...
def plot_eff_maps(all_hist, hit_hist, extent, module, plot_dir):
    ...
    if not os.path.exists(plot_dir + '/' + str(module) + '_iogroup_cbar_5_x_5_pixel_overview.pdf'):
        plt.savefig(plot_dir + '/' + str(module) + '_iogroup_cbar_5_x_5_pixel_overview.pdf', format = 'pdf')
...
def HoughOnArray(hits, id, min_pixels=125, MAX_peaks=100):
    ...
    file_in = 'coordinates' + str(id) + '.csv'
    file_out = "./hough3D_outputfile" + str(id) +".csv"

    np.savetxt(file_in, hits[0][['x', 'y', 'z']], delimiter=',', header='x,y,z')
    ...
    return float(aX), float(aY), float(aZ), float(bX), float(bY), float(bZ), int(npoints), float(xh_i), float(xh_f), float(yh_i), float(yh_f), float(zh_i), float(zh_f)
'''
...
def trackLengthhoughcenter(hits, aX, aY, aZ, bX, bY, bZ, radius, dist):
    ...
    hits_cp = np.array([hits[0]['x'], hits[0]['y'], hits[0]['z'], hits[0]['Q'], hits[0]['E'], hits[0]['id']])
    mask = dist < radius
    hits_cp = hits_cp.T[mask]
    dist_cp = dist[dist<radius]
    ...
    return hits_cp, dist_cp, np.max(avg_dist_2), aX_nl, aY_nl, aZ_nl
