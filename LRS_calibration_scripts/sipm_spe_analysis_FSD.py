...
def fit_lump_spectrum(hist, bins, npe=N_PHOTOELECTRONS, A0=None, mu0=None, sigma0=None, q0=None, p0=None):
    ...
    if q0<2e3:
        popt, pcov = curve_fit(
            ...
            hist[:-1],
            p0=p0,
            bounds=([p0[0]-(0.25*p0[0]), 0.05, 2.5e3*0.7, 1, 0.01, 0.2*2.5e3, 1], [p0[0]+(0.25*p0[0]), 5, 2.5e3*1.3, 1e3, 1.1, 0.45*2.5e3, 1e3])
        )
    else:
        popt, pcov = curve_fit(
            lambda q, A1, mu, q_spe, sigma_spe, A_lump, mu_lump, sigma_lump: spe_model(q, A1, mu, q_spe, sigma_spe, A_lump, mu_lump, sigma_lump, A0, mu0, sigma0, q0, npe)[0],
            ...
            hist[:-1],
            p0=p0,
            bounds=([p0[0]-(0.25*p0[0]), 0.05, q0*0.7, 1, 0.01, 0.2*q0, 1], [p0[0]+(0.25*p0[0]), 5, q0*1.4, 1e3, 1.1, 0.45*q0, 1e3])
        )
...
def analyze_spe(file_path, output_file):
    ...
    gain_outputs = np.zeros((4,64,2), dtype=np.float64)
    #with h5py.File(file_path, "r") as f
    waveform_data = np.load(file_path)
    waveforms = waveform_data[list(waveform_data.keys())[0]]
    print('Waveforms shape:', np.shape(waveforms))
    with open('/global/cfs/cdirs/dune/users/ajwhite/2x2_LRS_DataAssess/2025_Calibration/AFIViewer/WaveformCalib_FSD.yaml', 'r') as file:
    ...
    with PdfPages(pdf_filname) as pdf_output:
        ...
        #ch = 57
                ...
                if len(wfs_v1[charge_mask==1]) > 0:
                    ...
                    ped_integrals = np.trapz(wfs[:,0:50], axis=-1) #/ 5.01187
                    sig_integrals = np.trapz(wfs[:,60:90], axis=-1) #/ 5.01187
                    sig_subtracted = sig_integrals - (np.mean(wfs[:,0:50], axis=-1)*30)
                    del sig_integrals
                    ...
                    #plt.figure(figsize=(10, 5))
                        ...
                        axs[0].set_ylim(-100,np.int32(popt_spe[0])*3)
                        axs[0].legend()
                        axs[0].grid(True)
...
def main(input_file, output_file):
   ...
   np.savez('/global/cfs/cdirs/dune/users/ajwhite/2x2_LRS_DataAssess/2025_Calibration/AFIViewer/Output_FSD_01/v01_gain_chi2_dc.npz', data=gain_values)
