...
class ClusterMaker(H5FlowStage):
    '''
        Module to cluster hits using DBSCAN.
    
        Example config:
            classname: ClusterMaker # reco/charge/cluster_maker.py
            path: proto_nd_flow.reco.charge.cluster_maker
            requires:
              - 'charge/events'
        ...
        ``clusters`` datatype::
            ...
            Q              f8, hit charge [ke-] 
            E              f8, hit energy [MeV]
            sole_io_group  u8, io group if cluster is only contained in one io group (-1 if not)
    ...
    def init(self, source_name):
        ...
        self.charge_events_hits_dset = self.data_manager.get_ref_region(source_name, self.hits_name)
        self.hits = self.data_manager.get_dset(self.hits_name)
        #self.charge_events_hits_dset, _ = self.data_manager.get_ref(source_name, self.hits_name)
        #self.hits_ref_dset, _ = self.data_manager.get_ref(source_name,self.hits_name)
        
    def run(self, source_name, source_slice, cache):
        super(ClusterMaker, self).run(source_name, source_slice, cache)
        events = cache[source_name]
        for i in range(len(events)):
            indices = self.charge_events_hits_dset[events[i]['id']]
            start, stop = indices[0], indices[1]            
            hits = self.hits[start:stop]
            if not len(hits):
                continue

            hits = hits[~hits['is_disabled']]
            hits_ev_id = np.ones(len(hits))*events[i]['id']
            hits_unix_ts = np.ones(len(hits))*events[i]['unix_ts']
            labels = np.zeros((0,), dtype='int')
            hits_temp = np.zeros((0,), dtype=hits.dtype)
            hits_ev_id_temp = np.zeros((0,))
            hits_unix_ts_temp = np.zeros((0,))

            hits_mask = ~np.isnan(hits['z']) & ~np.isnan(hits['y'])
            if not np.sum(hits_mask):
                continue
            hit_coordinates = np.hstack((hits[hits_mask]['z'][:, np.newaxis], \
                                         hits[hits_mask]['y'][:, np.newaxis], \
                                         hits[hits_mask]['x'][:, np.newaxis], \
                                         hits[hits_mask]['ts_pps'][:, np.newaxis]))
            ...
            clusters_sole_io_group = []
            for io_list in label_io_group: 
                # hot fix to save io group for clusters isolated in single io groups,
                # i.e. low energy events, for easy plotting and masking later. 
                if len(set(io_list)) == 1:
                    clusters_sole_io_group.append(list(set(io_list))[0])
                else:
                    clusters_sole_io_group.append(-1)
            #io_group_clusters = np.array(list(map(np.min, label_io_group)))
            clusters_data['sole_io_group'] = np.array(clusters_sole_io_group)
