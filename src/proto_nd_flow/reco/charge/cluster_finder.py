...
class ClusterBuilder(H5FlowStage):
    ...
    def run(self, source_name, source_slice, cache):
        ...
        charge_event_hits_indices = self.charge_events_hits_dset[events['id']]
        total_hits = 0
        for i, indices in enumerate(charge_event_hits_indices):
            start, stop = indices[0], indices[1]
            #print(events[i]['id'])
            
            #hits = cache[self.hits_name][~rfn.structured_to_unstructured(cache[self.hits_name].mask).any(axis=-1)][start:stop]
            hits = self.hits[start:stop]
            total_hits+=len(hits)
            if not len(hits):
                continue
            hits_ev_id = np.ones(len(hits))*events[i]['id']
            #hits_ev_id = self.charge_events_hits_dset[:,0][np.isin(self.charge_events_hits_dset[:,1], hits['id'])]
            ...
            if not np.sum(hits_mask):
                continue
            hit_coordinates = np.hstack((hits[hits_mask]['z'][:, np.newaxis], hits[hits_mask]['y'][:, np.newaxis], hits[hits_mask]['x'][:, np.newaxis], \
