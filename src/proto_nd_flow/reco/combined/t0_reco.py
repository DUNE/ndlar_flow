...
class T0Reconstruction(H5FlowStage):
    ...
    def __init__(self, **params):
        ...
        self.raw_hits_dset_name = params.get('raw_hits_dset_name', self.default_ext_trigs_dset_name)
    ...
    def init(self, source_name):
        ...
        # create raw_hits -> t0 refs
        self.data_manager.create_ref(self.raw_hits_dset_name, self.t0_dset_name)
    ...
    def run(self, source_name, source_slice, cache):
        ...
        ## should put in a referrence between t0 and raw_hits..
        #ev_id = np.arange(source_slice.start, source_slice.stop, dtype=int).reshape(-1, 1)
        #raw_hits_ev_id = np.broadcast_to(ev_id, raw_hits_data.shape)
        #ref = np.c_[hits_ev_id[hits_mask], raw_hits_data[hits_mask]['id']]
        #self.data_manager.write_ref(self.raw_hits_dset_name, self.t0_dset_name, ref)
