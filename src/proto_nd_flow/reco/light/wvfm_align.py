...
class WaveformAlign(H5FlowStage):
    '''
        ...
        Parameters:
         ...
         - ``sim_latency`` : ``int``, optional, #samples in waveform data before triggger in simulation light
    ...
    def __init__(self, **params):
        ...
        self.sim_latency = params.get('sim_latency', self.default_sim_latency)
    ...
    def run(self, source_name, source_slice, cache):
        ...
        if resources['RunData'].is_mc:
            align_data['sample_idx'] = np.full((wvfm_data.shape[0],self.nadc),self.sim_latency)
            i_event_offset = np.zeros((wvfm_data.shape[0],self.nadc,self.nchannel)).astype(int)
            align_data['ns'] = np.median(event_data['tai_ns'],axis=-1).ravel()
