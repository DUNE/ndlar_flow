    ...
    def run(self, source_name, source_slice, cache):
        ...
        if self.gen_signal_spectrum:
            ...
            if np.any(mask):
                ...
                spectrum = ma.array(np.abs(signal_fft)**2, mask=~np.broadcast_to(mask & wvfm_mask, signal_fft.shape))
                spectrum = spectrum.mean(axis=0)
                spectrum[spectrum.mask] = 0.
                spectrum[np.isnan(spectrum)] = 0
                n = np.count_nonzero(mask & wvfm_mask, axis=0)
        ...
        if self.gen_signal_impulse:
            ...
            if np.any(mask):
                ...
                impulse = ma.array(aligned_wvfms, mask=~np.broadcast_to(mask & wvfm_mask, aligned_wvfms.shape))
                impulse = impulse.mean(axis=0)
                impulse[impulse.mask] = 0
                impulse[np.isnan(impulse)] = 0
                n = np.count_nonzero(mask & wvfm_mask, axis=0)
